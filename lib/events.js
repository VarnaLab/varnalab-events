  ...
  var api = {
    ...
    updated: (events) => events
      .filter((event) => ((ev = db[ids.indexOf(event.id)]) =>
        ...
        (ev.cover_desktop || '').replace(/(.*)\?.*/, '$1') !==
        (event.cover_desktop || '').replace(/(.*)\?.*/, '$1') ||
        (ev.cover_mobile || '').replace(/(.*)\?.*/, '$1') !==
        (event.cover_mobile || '').replace(/(.*)\?.*/, '$1')
      )())
    ,
    fields: (events) => events
      .map(
        ({
          id, name, description, cover_desktop, cover_mobile,
